...
export const CartProvider: React.FC<CartProviderProps> = ({ children }) => {
  const [cartState, setCartState, { error, loading }] =
    useLocalStorage<CartState>(CART_STORAGE_KEY, defaultCartState);
    ...
    (
      ...
      updateCartState((prev) => {
        ...
        const existingItemIndex = session
          ...
          : prev.items.findIndex(
              (item) => item.program.id === program.id && !item.selectedSession
            );
        ...
        if (existingItemIndex >= 0) {
          ...
          newItems[existingItemIndex] = {
            ...
            paymentPlan:
              options?.paymentPlan ?? newItems[existingItemIndex].paymentPlan,
          };
  ...
  const removeFromCart = useCallback(
    (programId: number, sessionId?: string) => {
      updateCartState((prev) => ({
        ...prev,
        items: prev.items.filter((item) => {
          if (sessionId) {
            ...
            return !(
              ...
              item.selectedSession?.id === sessionId
            );
          } else {
  ...
  const updateQuantity = useCallback(
    (programId: number, quantity: number, sessionId?: string) => {
      if (quantity <= 0) {
      ...
      updateCartState((prev) => ({
        ...
        items: prev.items.map((item) => {
          if (sessionId) {
            ...
            return item.program.id === programId &&
              item.selectedSession?.id === sessionId
              ? { ...item, quantity }
              : item;
          } else {
  ...
  // Clear cart and add single item (for direct enrollment)
  const clearAndEnroll = useCallback(
    (
      program: Program,
      quantity: number = 1,
      session?: SessionType,
  ...
  const getItemCount = useCallback(
    (programId: number, sessionId?: string): number => {
