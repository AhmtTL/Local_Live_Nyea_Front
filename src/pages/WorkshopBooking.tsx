...
const premierWorkshop = {
    ...
    description: "An exclusive two-day intensive leadership experience led by Ivy League Instructor Nicholas Coburn-Palo",
    ...
    features: [
        ...
        "Learn elite leadership vocabulary and precise word choice",
...
const WorkshopBooking: React.FC = () => {
    ...
    const [selectedProgramType, setSelectedProgramType] = useState<"mastery" | "essentials" | "all">("all");
        ...
        const loadWorkshopData = async () => {
            try {
                ...
                const slugMapping: { [key: string]: string } = {
                    "leadership-negotiation-communication": "harvard-negotiation-workshop",
                ...
                const programSlug = workshop ? slugMapping[workshop] || "premier-workshop" : "premier-workshop";

                // Get schid from URL parameters if present
                const schid = searchParams.get("schid");

                const response = await apiClient.getWorkshopSessions(programSlug, schid || undefined);

                if (response.success) {
                    // Transform backend data to frontend format
                    const transformedSessions: WorkshopSession[] = response.data.sessions.map((session: any) => ({
                    ...
                    setWorkshopProgram({
                        ...
                        description: response.data.program.description || premierWorkshop.description,
                ...
                setError(err instanceof Error ? err.message : "Failed to load workshop data");
            } finally {
                setLoading(false);
            }
        };
    ...
    const handleEnrollNow = async (session: WorkshopSession) => {
        if (!workshopProgram) return;

        setIsBooking(true);

        // Clear cart and enroll in this session
        clearAndEnroll(workshopProgram, 1, session);
        ...
        <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50/30 to-indigo-50/50">
            ...
            <section className=" relative overflow-hidden bg-gradient-to-br from-[#1f2444] via-[#2d3d66] to-[#9f162e] text-white">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 md:py-16">
                    <Button variant="outline" asChild className="mb-6 border-gray-300 bg-white text-gray-700 hover:bg-gray-50 hover:text-gray-900">
                    ...
                    <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
                        <div className="flex items-center mb-4">
                            ...
                            <span className="text-yellow-400 font-semibold">Premier Workshop</span>
                        </div>
                        <h1 className="text-3xl md:text-4xl lg:text-5xl font-bold mb-4 leading-tight">{workshopProgram?.title || premierWorkshop.title}</h1>
                        <p className="text-lg md:text-xl text-gray-200 max-w-3xl leading-relaxed">{workshopProgram?.description || premierWorkshop.description}</p>
                    </motion.div>
                </div>
            </section>
                    ...
                    {loading ? (
                        <div className="flex justify-center items-center py-16">
                            <div className="flex items-center space-x-4">
                                ...
                                <span className="text-lg text-gray-600">Loading workshop sessions...</span>
                            </div>
                        </div>
                    ) : error ? (
                        <div className="text-center py-16">
                            <div className="bg-red-50 border border-red-200 rounded-lg p-6 max-w-md mx-auto">
                                <h3 className="text-lg font-semibold text-red-800 mb-2">Error Loading Workshop</h3>
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8">
                            ...
                            <div className="lg:col-span-2">
                                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
                                    <h2 className="text-xl sm:text-2xl md:text-3xl font-bold text-gray-900 mb-4 sm:mb-6">Select Your Session</h2>
                                    <p className="text-sm sm:text-base text-gray-600 mb-4 sm:mb-6">Choose from our upcoming workshop sessions in premium locations worldwide.</p>
                                        ...
                                        <div className="flex flex-col sm:flex-row gap-2 sm:gap-3">
                                            <button
                                                ...
                                                className={`px-3 sm:px-4 py-2 rounded-lg font-medium transition-all text-sm sm:text-base ${
                                                    selectedProgramType === "all" ? "bg-[#1f2444] text-white shadow-lg" : "bg-gray-100 text-gray-700 hover:bg-gray-200"
                                                }`}
                                            >
                                            ...
                                            <button
                                                ...
                                                className={`px-3 sm:px-4 py-2 rounded-lg font-medium transition-all flex items-center justify-center text-sm sm:text-base ${
                                                    selectedProgramType === "essentials" ? "bg-blue-600 text-white shadow-lg" : "bg-gray-100 text-gray-700 hover:bg-gray-200"
                                                }`}
                                            >
                                                <span className="sm:hidden">⚡ Essentials ({workshopSessions.filter(s => s.program_type === "essentials").length})</span>
                                                <span className="hidden sm:inline">⚡ Essentials ({workshopSessions.filter(s => s.program_type === "essentials").length})</span>
                                            </button>
                                            <button
                                            ...
                                            >
                                                <span className="sm:hidden">👑 Mastery ({workshopSessions.filter(s => s.program_type === "mastery").length})</span>
                                                <span className="hidden sm:inline">👑 Mastery ({workshopSessions.filter(s => s.program_type === "mastery").length})</span>
                                            </button>
                                        </div>
                                    </div>
                                    ...
                                    <div className="space-y-4">
                                        {workshopSessions
                                            .filter(session => {
                                                // Program type filter
                                                const programTypeMatch = selectedProgramType === "all" || session.program_type === selectedProgramType;

                                                // Search filter
                                                const searchMatch =
                                                    searchQuery === "" ||
                                                    session.location.toLowerCase().includes(searchQuery.toLowerCase()) ||
                                                    session.venue_name?.toLowerCase().includes(searchQuery.toLowerCase()) ||
                                            ...
                                            .sort((a, b) => {
                                                ...
                                                if (a.location.includes("Al Faris International School") && !b.location.includes("Al Faris International School")) return -1;
                                                if (!a.location.includes("Al Faris International School") && b.location.includes("Al Faris International School")) return 1;
                                                return 0;
                                            })
                                            .map((session, index) => (
                                                ...
                                                >
                                                    <SessionCard
                                                        ...
                                                        onCardClick={() => {
                                                            navigate(`/workshop-booking/${workshop}/session/${session.unique_identifier || session.id}`);
                                                        }}
                                                        onAddToCart={() => handleEnrollNow(session)}
                                                        onRemoveFromCart={() => handleRemoveFromCart(session)}
                                                        isInCart={isSessionInCart(session.id)}
                                                        isBooking={isBooking}
                                                        getBadge={(highlights?: string[]) => getAudienceBadge(highlights || [])}
                                                        getGradeInfo={(highlights?: string[], metadata?: any) => getGradeInfo(highlights, metadata)}
                                    ...
                                    <Card className="shadow-lg lg:shadow-xl border-0 bg-white">
                                        ...
                                        <CardContent className="space-y-6">
                                            ...
                                            <div>
                                                ...
                                                />
                                                <h3 className="font-bold text-gray-900 mb-2">{workshopProgram?.title || premierWorkshop.title}</h3>
                                                <p className="text-sm text-gray-600 mb-4">{workshopProgram?.description || premierWorkshop.description}</p>

                                                {/* Features */}
                                                <div className="space-y-2">
                                                    {(workshopProgram?.features || premierWorkshop.features).slice(0, 4).map((feature: string, index: number) => (
                                            ...
                                            {/* Cart CTA */}
                                            {/* View Cart button - Disabled for better UX (user goes directly to checkout) */}
                                            {/* {items.length > 0 && (
                              ...
                              View Cart ({items.length})
                                            ...
                                            <div className="border-t pt-6 space-y-3">
                                                ...
                                                <div className="flex items-center text-sm text-gray-600">
                                                    ...
                                                    {/* Ivy League University Instructor */}
                                                    Led by Instructor
                                                </div>
                                                <div className="flex items-center text-sm text-gray-600">
            ...
            <section className="py-12 bg-gray-50">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="text-center mb-8">
                        <h2 className="text-2xl md:text-3xl font-bold text-gray-900 mb-4">Information about the Program Format</h2>
                        <p className="text-gray-600 max-w-2xl mx-auto">We offer two program formats to suit different learning preferences and schedules</p>
                    </motion.div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-4xl mx-auto">
                        ...
                        >
                            <div className="flex items-center mb-4">
                                <div className="bg-gradient-to-r from-blue-400 to-blue-600 text-white text-xs px-3 py-1 rounded-full font-bold">⚡ ESSENTIALS</div>
                            </div>
                            <h3 className="text-lg font-bold text-gray-900 mb-2">2-Day Intensive Program</h3>
                            <p className="text-gray-600 text-sm mb-4">Focused on core negotiation and leadership skills in an intensive format. Perfect for busy professionals.</p>
                            <ul className="space-y-2 text-sm text-gray-600">
                                <li className="flex items-center">
                                    <CheckCircle className="h-4 w-4 text-green-600 mr-2" />3 days of intensive training
                                </li>
                                <li className="flex items-center">
                        ...
                        >
                            ...
                            <div className="flex items-center mb-4">
                                <div className="bg-gradient-to-r from-amber-400 to-orange-500 text-white text-xs px-3 py-1 rounded-full font-bold">👑 MASTERY</div>
                                <span className="ml-2 text-xs text-amber-700 font-semibold">PREMIUM</span>
                            </div>
                            <h3 className="text-lg font-bold text-gray-900 mb-2">7-Day Extended Experience</h3>
                            <p className="text-gray-600 text-sm mb-4">Comprehensive program with personalized coaching, exclusive networking, and advanced methodologies.</p>
                            <ul className="space-y-2 text-sm text-gray-600">
                                <li className="flex items-center">
                                    <CheckCircle className="h-4 w-4 text-green-600 mr-2" />
