...
import { ArrowLeft, Calendar, MapPin, Users, CheckCircle, Shield, Star, Loader2, ShoppingCart, X, GraduationCap, Share2, Check, CheckCircle2 } from "lucide-react";
import { Button } from "../components/ui/button";
import { Card, CardContent } from "../components/ui/card";
import { useCart } from "../context/CartContext";
import apiClient from "../services/api";
import { getTimeSuffix } from "../utils/time";
...
const premierTrainingCamp = {
    ...
    description: "An intensive leadership and entrepreneurship training program designed to develop the next generation of business leaders",
    price: 799,
    duration: "5 Day Intensive",
    category: "Leadership Training",
    image: "/images/future-ceo.jpeg",
    features: [
        "Ivy League Business School Curriculum",
...
const TrainingCampSessionDetail: React.FC = () => {
    ...
    const { clearAndEnroll, removeFromCart, items } = useCart();
    ...
    useEffect(() => {
        const loadSessionData = async () => {
            try {
                ...
                const response = await apiClient.getTrainingCampSession(actualSessionId);

                if (response.success && response.data) {
                    const sessionData = response.data;

                    // Calculate capacity percentage if not provided
                    const totalSpots = (sessionData.booked_spots || 0) + sessionData.remaining_spots;
                    const bookedSpots = sessionData.booked_spots || 0;
                    const capacityPercentage = sessionData.capacity_percentage || (totalSpots > 0 ? (bookedSpots / totalSpots) * 100 : 0);
                    ...
                    const transformedSession: TrainingCampSession = {
                        ...
                        is_almost_sold_out: sessionData.is_almost_sold_out || capacityPercentage >= 90,
                        is_limited_availability: sessionData.is_limited_availability || (capacityPercentage >= 75 && capacityPercentage < 90),
                    ...
                    setTrainingCampProgram({
                        ...
                        description: programData.description || premierTrainingCamp.description,
                        price: programData.price ? parseFloat(programData.price) : premierTrainingCamp.price,
    ...
    const calculateDepositPlan = (sessionData: TrainingCampSession | null, useInstallment: boolean = false) => {
        ...
        const totalPrice = sessionData.price ?? trainingCampProgram?.price ?? premierTrainingCamp.price;
    ...
    const isSessionInCart = () => {
        return session ? items.some(item => item.selectedSession?.id === session.id) : false;
    };
    ...
    const getGradeInfo = (locationHighlights?: string[], metadata?: any) => {
        const gradeHighlight = locationHighlights?.find(highlight => highlight.includes("Grade"));
    ...
    const depositPercentageDisplay = depositPlan ? Math.round((depositPlan.metadata?.deposit_percentage ?? 0.3) * 100) : null;
    const timeSuffix = getTimeSuffix(session?.time);
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
                    <div className="flex justify-center items-center">
                        <div className="flex items-center space-x-4">
                            ...
                            <span className="text-lg text-gray-600">Loading session details...</span>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
                    <div className="text-center">
                        <div className="bg-red-50 border border-red-200 rounded-lg p-6 max-w-md mx-auto">
                            ...
                            <Button
                                onClick={() => navigate(`/ivy-league-future-ceo-booking/${trainingCamp}`)}
                                variant="outline"
        ...
        <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50/30 to-indigo-50/50">
            ...
            <section className="relative overflow-hidden bg-gradient-to-br from-[#1f2444] via-[#2d3d66] to-[#9f162e] text-white">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
                    <Button variant="outline" asChild className="mb-4 sm:mb-6 border-gray-300 bg-white text-gray-700 hover:bg-gray-50 hover:text-gray-900">
                        <Link to={`/ivy-league-future-ceo-booking/${trainingCamp}`}>
                    ...
                    <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
                        <div className="flex items-center mb-3 sm:mb-4 flex-wrap gap-2">
                            ...
                            <span className="text-yellow-400 font-semibold text-sm sm:text-base">Premier Training Camp Session</span>
                        ...
                        <h1 className="text-2xl sm:text-3xl lg:text-4xl font-bold mb-3 sm:mb-4 leading-tight">{session.session_title || session.location}</h1>
                        <p className="text-base sm:text-lg lg:text-xl text-gray-200 max-w-3xl leading-relaxed">
                            {trainingCampProgram?.description || premierTrainingCamp.description}
                        </p>
                    </motion.div>
                </div>
            </section>
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8">
                            ...
                            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="space-y-6">
                                    ...
                                    <Card className="border-2 border-gray-300 bg-gradient-to-r from-gray-50 to-gray-100">
                                        <CardContent className="p-6">
                                            <div className="flex items-center justify-center gap-3">
                                                ...
                                                <div className="text-center">
                                                    <h3 className="text-lg font-bold text-gray-800 mb-1">Session Completed</h3>
                                                    <p className="text-sm text-gray-600">This session has reached full capacity. Thank you for your interest!</p>
                                                </div>
                                            </div>
                                        </CardContent>
                                    </Card>
                                ...
                                <Card
                                    className={`border-2 relative overflow-hidden ${
                                        session.program_type === "intensive" ? "border-amber-200 bg-gradient-to-br from-amber-50/50 to-orange-50/50" : "border-gray-200"
                                    }`}
                                >
                                    <CardContent className="p-4 sm:p-6 lg:p-8 relative">
                                                ...
                                                <div className="text-right">
                                                    {depositPlan ? (
                                                        <div className="space-y-1">
                                                            ...
                                                            <div className="text-sm text-gray-500">Balance {formatCurrency(depositPlan.balanceAmount)} due later</div>
                                                            <div className="text-xs text-gray-400">Total programme fee {formatCurrency(depositPlan.totalAmount)}</div>
                                                        </div>
                                                    ) : (
                                                        <div className="text-2xl sm:text-3xl lg:text-4xl font-bold text-blue-600">
                                                            {session.formatted_price || `$${session.price?.toLocaleString()}`}
                                                        </div>
                                                    )}
                                                </div>
                                            </div>
                                        ...
                                        {session.special_notes && (
                                            <div className="mb-8 p-4 sm:p-6 bg-amber-50 rounded-xl border border-amber-200">
                                                ...
                                                <p className="text-sm sm:text-base text-amber-800 leading-relaxed">{session.special_notes}</p>
                                            </div>
                                        )}
                                            ...
                                            <div className="mb-8 p-4 sm:p-6 bg-green-50 rounded-xl border border-green-200">
                                                ...
                                                <div className="space-y-3">
                                                    <label className="flex items-center cursor-pointer">
                                                        ...
                                                        />
                                                        <div>
                                                            <span className="font-medium text-green-900">Pay in Full</span>
                                                            <p className="text-sm text-green-700">
                                                                Complete payment now -{" "}
                                                                {session?.formatted_price ||
                                                                    formatCurrency(session?.price ?? trainingCampProgram?.price ?? premierTrainingCamp.price)}
                                                            </p>
                                                        </div>
                                                    </label>
                                                    {/* Installment/deposit option removed */}
                                                </div>
                                            </div>
                                        )}

                                        {/* Deposit plan overview removed (installment option removed) */}
                                        ...
                                        <div className="space-y-5 mb-8">
                                            ...
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div className="flex-1">
                                                    {(session.venue_name || session.location) && (
                                                        <div className="font-semibold text-gray-700 text-base">{session.venue_name || session.location}</div>
                                                    )}
                                                ...
                                                <div className="flex-1">
                                                    <div className="font-semibold text-gray-700 text-base">
                                                        {session.start_date && session.end_date
                                                            ? `${formatDateRange(session.start_date, session.end_date)}${timeSuffix}`
                                                            : `${formatDate(session.date)}${timeSuffix}`}
                                                    </div>
                                                    {session.timezone && <div className="text-sm text-gray-400 font-light mt-0.5">{session.timezone}</div>}
                                                </div>
                                            </div>
                                            ...
                                            {getGradeInfo(session.location_highlights, session.metadata) && (
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <div className="flex-1">
                                                        <div className="font-semibold text-gray-700 text-base">{getGradeInfo(session.location_highlights, session.metadata)}</div>
                                                        <div className="text-sm text-gray-500 font-light mt-0.5">Academic Level</div>
                                                    </div>
                                                </div>
                                            )}
                                            ...
                                            <div className="flex items-start gap-3">
                                                ...
                                                <div className="flex-1">
                                                    <div className="flex items-center gap-3 mb-3">
                                                        {/* spots available text removed; keep FCFS badge if applicable */}
                                                        {session.availableSpots <= 15 && !session.is_almost_sold_out && !session.is_limited_availability && (
                                                    ...
                                                    {/* Capacity progress bar and enrolled count removed */}
                                                </div>
                                            </div>
                                        </div>

                                        {/* Enhanced Scarcity Indicators - Matching listing layout exactly */}
                                        {((session.availableSpots > 0 && session.is_almost_sold_out) || session.is_limited_availability) && (
                                            <div className="pt-2 mb-6">
                                                {session.is_almost_sold_out && (
                                                    <div className="inline-flex items-center px-4 py-2 bg-red-100 text-red-800 text-sm font-bold rounded-lg border border-red-200 animate-pulse">
                                                        🔥 <span className="font-extrabold">ALMOST SOLD OUT</span> - Only{" "}
                                                        <span className="font-extrabold">{session.availableSpots}</span> left!
                                                    </div>
                                                )}
                                                {!session.is_almost_sold_out && session.is_limited_availability && (
                                                    <div className="inline-flex items-center px-4 py-2 bg-orange-100 text-orange-800 text-sm font-bold rounded-lg border border-orange-200">
                                                        ⚡ <span className="font-extrabold">LIMITED AVAILABILITY</span> :{" "}
                                                        <span className="font-extrabold">{session.availableSpots}</span> spots left
                                                    </div>
                                                )}
                                            </div>
                                        )}
                                    ...
                                    {/* Session Completed Overlay - On Top */}
                                    {session.availableSpots <= 0 && (
                                        <div className="absolute inset-0 z-50 flex items-center justify-center pointer-events-none bg-gray-900/40 backdrop-blur-sm">
                                            <div className="bg-white/95 backdrop-blur-sm rounded-lg p-6 shadow-2xl text-center max-w-md mx-4">
                                                <CheckCircle2 className="h-12 w-12 text-gray-600 mx-auto mb-3" />
                                                <h3 className="text-xl font-bold text-gray-800 mb-2">Session Completed</h3>
                                                <p className="text-sm text-gray-600">This session has reached full capacity. Thank you for your interest!</p>
                                            </div>
                                        </div>
                                    )}
                        ...
                        <div className="lg:col-span-1">
                            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6, delay: 0.2 }} className="lg:sticky lg:top-6">
                                <Card className="shadow-lg border-0 bg-white">
                                    <CardContent className="p-4 sm:p-6 space-y-6">
                                        {/* Training Camp Image */}
                                        <div>
                                            <img
                                                src={trainingCampProgram?.image || premierTrainingCamp.image}
                                                alt={trainingCampProgram?.title || premierTrainingCamp.title}
                                                className="w-full h-48 object-cover rounded-lg mb-4"
                                            />
                                            <h3 className="font-bold text-gray-900 mb-2 text-base sm:text-lg">{trainingCampProgram?.title || premierTrainingCamp.title}</h3>
                                            <p className="text-xs sm:text-sm text-gray-600 mb-4">{trainingCampProgram?.description || premierTrainingCamp.description}</p>
                                        </div>

                                        {/* Features */}
                                        <div className="border-t pt-6 space-y-3">
                                            <h4 className="font-semibold text-gray-900 text-sm sm:text-base mb-3">What's Included</h4>
                                            {(trainingCampProgram?.features || premierTrainingCamp.features).map((feature: string, index: number) => (
            ...
            {/* Enroll Now and Share Buttons - Fixed to Viewport Bottom */}
            {session.availableSpots > 0 && (
                        ...
                        <div className="lg:max-w-5xl lg:mx-auto bg-white border border-gray-200 shadow-xl rounded-xl p-3 sm:p-4 lg:p-5">
                            <div className="flex flex-col sm:flex-row gap-3">
                                ...
                                <Button
                                    ...
                                    disabled={isBooking}
                                    size="lg"
                                    className={`flex-1 bg-gradient-to-r from-[#9f162e] to-[#1f2444] hover:from-[#1f2444] hover:to-[#9f162e] text-white font-bold py-4 text-base sm:text-lg shadow-lg transition-all duration-200 disabled:opacity-50`}
                                >
                                    {isBooking ? (
                                        <>
                                        ...
                                        <>
                                            ...
                                            <span className="">{useInstallmentPayment ? "Reserve with Deposit" : "Enroll Now"}</span>
                                        </>
                                    )}
                                </Button>
                                ...
                                <Button
                                    ...
                                    className={`sm:w-auto px-6 py-4 font-bold transition-all duration-200 ${
                                        copied ? "bg-green-50 border-green-500 text-green-700 hover:bg-green-100" : "border-gray-300 text-gray-700 hover:bg-gray-50"
                                    }`}
                                >
                                    {copied ? (
                                        <>
