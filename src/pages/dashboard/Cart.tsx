...
const Cart: React.FC = () => {
  ...
  // Auto-open modal if autoOpen parameter is present
  useEffect(() => {
    if (searchParams.get('autoOpen') === 'true' && items.length > 0 && !guestInfo) {
      setShowGuestModal(true);
      // Remove the autoOpen parameter from URL
      searchParams.delete('autoOpen');
      setSearchParams(searchParams, { replace: true });
    }
  }, [searchParams, setSearchParams, items.length, guestInfo]);
  ...
  const handleGuestInfoSubmit = async (info: GuestInfo) => {
    setGuestInfo({
      ...
      parentPhoneNumber: info.parentPhoneNumber,
    });
    ...
    try {  
      ...
      await createCheckoutSession(items, guestInfoForBackend);
    } catch (err) {
  ...
  const handleCheckout = async () => {
    // Check if cart contains Ivy League Negotiation Workshop without a selected session
    const hasNegotiationWorkshopWithoutSession = items.some((item) => {
      const programSlug = (item.program as any)?.slug;
      const programTitle = item.program.title?.toLowerCase();
      ...
      // If it's negotiation workshop and no session selected, redirect
      return isNegotiationWorkshop && !item.selectedSession;
    });

    if (hasNegotiationWorkshopWithoutSession) {
      // Redirect to workshop booking page to choose program format
    ...
    try {
      // Combine firstName and lastName into name for backend
      const guestInfoForBackend = guestInfo
        ? {
            ...
            parentEmail: guestInfo.parentEmail,
            parentPhoneNumber: guestInfo.parentPhoneNumber,
          }
        : undefined;
      await createCheckoutSession(items, guestInfoForBackend);
    } catch (err) {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6 lg:gap-8">
                          ...
                          {item.selectedSession ? (
                             <div className="bg-gray-50 border border-gray-200 rounded-lg sm:rounded-xl p-3 sm:p-4 mb-3 sm:mb-4 relative pb-12 sm:pb-8">
                               ...
                               <div className="space-y-1.5 sm:space-y-2">
                                 ...
                                 <div className="flex items-start sm:items-center gap-1.5 sm:gap-2">
                                   ...
                                   <div className="flex-1 min-w-0">
                                     <span className="font-medium text-gray-700 text-xs sm:text-sm break-words">
                                      {(() => {
                                        const dateText = item.selectedSession.formatted_date || formatDate(item.selectedSession.date);
                                        const ct = cleanTimeString(item.selectedSession.time);
                                        return ct ? `${dateText} · ${ct}` : `${dateText}`;
                                      })()}
                                     </span>
                                 ...
                                 {/* Availability removed per design: spots + progress bar omitted */}
                               </div>
          ...
          <div className="lg:col-span-1">
            <div className="bg-white rounded-xl sm:rounded-2xl shadow-xl p-4 sm:p-6 lg:p-8 lg:sticky lg:top-8">
              ...
              {/* Checkout Button - Auto-open guest form */}
              {!guestInfo && (
              ...
              {guestInfo && (
                <div className="mb-4 sm:mb-6 p-3 sm:p-4 bg-green-50 border border-green-200 rounded-lg sm:rounded-xl">
                  <div className="flex items-center mb-1.5 sm:mb-2">
                    ...
                    <h4 className="text-sm sm:text-base font-semibold text-green-900">Checkout Information</h4>
                  </div>
                  <p className="text-xs sm:text-sm text-green-800 break-words">{guestInfo.firstName}{guestInfo.lastName}</p>
                  <p className="text-xs sm:text-sm text-green-700 break-words">{guestInfo.email}</p>
                </div>
              )}
