...
import { Calendar, ShoppingCart, X, Users, GraduationCap, CheckCircle2 } from "lucide-react";
...
export const SessionCard: React.FC<SessionCardProps> = ({
  ...
  // `cleanTimeString` imported from shared util
  ...
  return (
    ...
    >
      <CardContent className="p-0 relative overflow-hidden w-full">
        {/* Session Completed Badge - Top Left - When capacity is full */}
        {session.availableSpots <= 0 && (
          <div className="absolute top-2 left-2 sm:top-3 sm:left-3 z-20">
            <div className="bg-gradient-to-r from-gray-700/80 to-gray-900/80 backdrop-blur-sm text-white px-3 py-1.5 sm:px-4 sm:py-2 rounded-lg shadow-lg flex items-center gap-2 text-xs sm:text-sm font-bold">
                ...
                <div className="flex items-center gap-x-1">
                  ...
                  <span className="text-[10px] md:text-sm text-[#6B7280]">
                      {session.start_date && session.end_date && formatDateRange
                        ...
                        : ""}
                      {(() => {
                        const ct = cleanTimeString(session.time);
                        return ct ? ` · ${ct}` : "";
                      })()}
                  </span>
                </div>
              ...
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-y-4 md:gap-x-10 w-full">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between w-full">
                    {/* Session Completed Message - Mobile */}
                    {session.availableSpots <= 0 && (
                ...
                {!hideCartButton && session.availableSpots > 0 && (
                  <Button
                    ...
                    className="w-full sm:w-auto flex-shrink-0 bg-gradient-to-r from-[#9f162e] to-[#1f2444] hover:from-[#1f2444] hover:to-[#9f162e] text-white rounded-[6px] font-medium h-10 px-6 text-sm shadow-lg transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed whitespace-nowrap"
                  >
                    {isBooking ? (
                      <>
